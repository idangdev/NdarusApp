...
class AlIkhlashActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivityAlIkhlashBinding.inflate(layoutInflater)
        setContentView(binding.root)

        val viewModel = ViewModelProvider(this, ViewModelProvider.NewInstanceFactory()).get(
            AlIkhlashViewModel::class.java)
            ...
            override fun onResults(bundle: Bundle) {
                ...
                if (result != null) {
                    if (current_position < listAlIkhlash.size - 1) {
                        ...
                        score++
                        Toast.makeText(applicationContext, "Benar", Toast.LENGTH_SHORT).show()
                        val intent = Intent(applicationContext, AlIkhlashDoneActivity::class.java)
                        intent.putExtra(AlIkhlashDoneActivity.EXTRA_SCORE, score)
                        intent.putExtra(AlIkhlashDoneActivity.EXTRA_JUMLAH_AYAT, jumlahAyat)
